...
passport.use(new GitHubStrategy({
    ...
    try {
        ...
        req.login(savedUser, (err: any) => { // Ensure you are passing the `req` object in the strategy callback
            if (err) { 
                console.error('Login failed', err);
                return done(err);
            }
            console.log('User logged in and saved:', savedUser);
            return done(null, savedUser);
        }) as User; // Cast the result of req.login to User

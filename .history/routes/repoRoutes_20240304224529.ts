...
// Assuming the necessary imports and setup are done above
const analyzeGithubUrl = async (req: Request, res: Response) => {
  const { githubUrl } = req.body;
  ...
  try {
    const repoDetails = await octokit.repos.get({ owner, repo });
    const promptText = `Analyze the GitHub repository "${owner}/${repo}" and provide a summary.`;
    // Analysis logic here
    return res.json({ analysis: "Mock analysis result", repoDetails: repoDetails.data });
  } catch (error) {
    // If error is due to repo not found or access issue, guide the user for authentication
    if ((error as any).status === 404 || (error as any).status === 403) {
      // Instead of redirecting, provide an instruction or URL for manual authentication initiation
      return res.status(401).json({
        error: "Authentication required. Please authenticate via GitHub to access private repositories.",
        authUrl: "/auth/github" // This should be adjusted based on your server setup
      });
    } else {
      return res.status(500).json({ error: "An unexpected error occurred" });
    }

        ...
        req.session.save(err => {
            if(err) {
                console.error('Session save error:', err);
                    ...
                    // console githubid
                    console.log    res.send(`successfully saved session ${githubId}`);
...
router.get('/githubid', (req, res) => {
  setCustomSessionProperty(req.session, 'githubId', '12345555');
  ...
  req.session.save(err => {
    ...
    res.send(`successfully saved session ${githubId}`);
  });
});

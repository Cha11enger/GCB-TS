...
import { sendMessageToGPTInterface } from '../utils/gptInterface';
...
// Function to initiate repository analysis
const initiateAnalysisForRepo = async (githubUrl: string) => {
  const pathRegex = /github\.com\/([^\/]+)\/([^\/]+)/;
  const match = githubUrl.match(pathRegex);

  if (!match) {
    sendMessageToGPTInterface({ success: false, message: "Invalid GitHub URL" });
    return;
  }
  ...
  try {
    ...
    const promptText = `Analyze the GitHub repository "${owner}/${repo}" and provide a summary of its main features, technologies used, and overall purpose.`;
    const analysisResult = await analyzeTextWithGPT(promptText);
    sendMessageToGPTInterface({ success: true, analysis: analysisResult, repoDetails: repoDetails.data });
  } catch (error) {
    sendMessageToGPTInterface({ success: false, message: "Authentication required to access this repository. Please authenticate via GitHub." });
  }
};

export const repoRoutes = {
  initiateAnalysisForRepo,
};

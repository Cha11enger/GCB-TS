...
async function checkRepoVisibility(owner: string, repo: string): Promise<string> {
  try {
    const response = await Octokit.rest.repos.get({ owner, repo }); // Access the 'repos' property from 'Octokit.rest'
...
const analyzeGithubUrl = async (req: Request, res: Response) => {
  ...
  // next check if its private or public repo using octokit
  const 

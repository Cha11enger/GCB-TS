...
router.get('/githubid', (req, res) => {
    ...
    // Directly access the property for debugging
    const githubIdDirect = req.session.cookie;
    console.log('GitHub ID from session (direct):', githubIdDirect);
    
    // Continue using your utility function as well
    const githubId = getCustomSessionProperty<string>(req.SessionData.git, 'githubId');
    console.log('GitHub ID from session (util):', githubId);
    
    if (!githubId) {
        return res.status(401).send('Unauthorized: No GitHub session found.');
    }
    
    // Proceed with your logic using req.session.githubId and req.session.accessToken
});

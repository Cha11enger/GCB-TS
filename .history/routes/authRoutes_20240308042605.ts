...
export const handleGitHubCallback = (req: Request, res: Response, next: NextFunction) =>
  passport.authenticate('github', { failureRedirect: '/auth/github' }, (err: any, user: IUser, info: any) => {
    ...
    req.logIn(user, (err) => {
      ...
      // if res.status(200).json({ message: 'User authenticated successfully' }); res.redirect('/'); // Redirect to the callback URL of chatbot interface
      if (res.status(200).json({ message: 'User authenticated successfully' })); res.redirect('https://chat.openai.com/aip/g-01abc1339e19a0ba559ee408b09ef8fad06faa4e/oauth/callback'); // Redirect to the callback URL of chatbot interface

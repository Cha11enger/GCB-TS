...
const analyzeGithubUrl = async (req: Request, res: Response) => {
  ...
  let accessToken = process.env.GITHUB_PAT; // Start with the PAT for initial attempt

  try {
    // First attempt with PAT
    await fetchRepositoryDetails(owner, repo, accessToken);
    // If successful, proceed to analyze with PAT
    return proceedWithAnalysis(owner, repo, accessToken, res);
  } catch (patError) {
    // If PAT fails, check if it's due to repo not being public or accessible
    if (patError.status === 404 || patError.status === 403) {
      // Check if user session has an access token
      accessToken = (req.session as { user?: { accessToken: string } }).user?.accessToken;
      if (!accessToken) {
        // If no user access token, prompt for authentication
        return res.status(403).json({
          error: "Authentication required. Please authenticate to access private repositories.",
          authUrl: getGithubAuthUrl(),
        });
      }
      // Try again with user's access token
      try {
        await fetchRepositoryDetails(owner, repo, accessToken);
        // If successful, proceed to analyze with user's access token
        return proceedWithAnalysis(owner, repo, accessToken, res);
      } catch (userTokenError) {
        // If still failing, report access issues
        return res.status(userTokenError.status).json({
          error: "Repository not found or access denied. Please ensure you have access to this repository.",
        });
      }
    } else {
      // For other errors with PAT, return a generic error
      return res.status(500).json({ error: "Error fetching repository details." });
    }
  }
};

async function fetchRepositoryDetails(owner, repo, token) {
  const octokit = new Octokit({ auth: token });
  return octokit.repos.get({ owner, repo });
}

async function proceedWithAnalysis(owner, repo, accessToken, res) {
  ...
  res.json({ analysis: analysisResult, repoDetails: { owner, repo, accessToken } }); // Simplified repoDetails for illustration
}

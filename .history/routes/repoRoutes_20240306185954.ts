...
const analyzeGithubUrl = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!repoDetails.data.private) {
      // check db for githubid or username and access token
      const user = await User.findOne({ githubId: repoDetails.data.owner.id || username: repoDetails.data.owner.login });

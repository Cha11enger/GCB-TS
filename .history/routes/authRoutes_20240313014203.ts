...
router.get('/github/callback', passport.authenticate('github', { failureRedirect: '/api/auth/github' }), (req, res) => {
  ...
  // Make sure to save session here after successful authentication
  if (req.user) {
    const user: IUser = req.user; // Assuming req.user is populated by passport upon successful authentication
    ...
    req.session.save((err) => {
      ...
      // Redirect with code and state only after the session has been successfully saved
      res.redirect(`${process.env.OPENAI_CALLBACK_URL}?code=${code}&state=${state}`);
    });
  } else {
    // Handle failure case
    res.redirect('/api/auth/github'); // Adjust according to your failure handling
  }
});
